            ...
            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
                {ownedItems.map(item => (
                    <InventoryCard key={item.id} name={item.name} isEquipped={equippedTheme === item.id} onEquip={() => onEquipItem(item.id, 'theme')}>
                         <div class={`w-20 h-20 rounded-lg border-4 ${item.id === 'summer' ? 'bg-amber-200 border-sky-700' : 'bg-[#F3D39C] border-[#B58863]'}`}></div>
                    </InventoryCard>
                ))}
            </div>
        );
        ...
        <div class="min-h-screen bg-[#0F172A] text-white p-8 font-sans">
            <div class="w-full max-w-7xl mx-auto">
                <div class="flex justify-between items-center mb-8">
                    <h1 class="text-5xl font-bold">Inventory</h1>
...
const InventoryCard: React.FC<{name: string, isEquipped: boolean, onEquip: () => void, children: React.ReactNode}> = ({ name, isEquipped, onEquip, children }) => (
    <div class={`bg-[#1E293B] p-4 rounded-2xl flex flex-col items-center text-center border ${isEquipped ? 'border-cyan-400' : 'border-gray-700'}`}>
