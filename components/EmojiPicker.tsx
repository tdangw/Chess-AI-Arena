...
const EmojiPicker: React.FC<EmojiPickerProps> = ({ onSelect, ownedEmojis }) => {
    ...
    return (
        <div 
            class="absolute top-full mt-2 w-80 bg-[#1E293B] border border-gray-700 rounded-xl p-4 shadow-lg z-30 animate-fade-in-up"
            style={{ 
                left: '50%',
            ...
            <div class="grid grid-cols-5 gap-3">
                {availableEmojis.map(emojiItem => (
                    <button
                        ...
                        class="text-4xl rounded-lg hover:bg-slate-700 transition-colors p-2 transform hover:scale-110"
                    >
                        {emojiItem.char}
                    </button>
                ))}
